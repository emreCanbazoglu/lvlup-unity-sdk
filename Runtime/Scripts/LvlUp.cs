...
namespace LvlUp
{
    ...
    public static class LvlUpSDK
    {
        ...
        /// <summary>
        /// Provide this device's advertising ID (IDFA/GAID) so the backend can recognize
        /// developer/test devices server-side (see <see cref="IsDeveloperDevice"/>). It is sent as
        /// the X-Device-Ad-Id header on config fetches — never in the URL — and the dev-device list is
        ...
        /// <summary>
        /// True if the backend recognized this device as a developer/test device — i.e. the
        /// advertising ID supplied via <see cref="SetDeviceAdvertisingId(string)"/> matched the
        /// server-side dev-device list. The list itself is never sent to the client. Use this to
        /// unlock dev-only features (e.g. call SRDebug.Init()) instead of matching a broadcast
        /// dev_devices list locally. Returns false until Remote Config has loaded and for all
        /// non-dev devices, so check it after <see cref="Config.WaitUntilReady(Action{bool}, float)"/>.
        /// </summary>
        public static bool IsDeveloperDevice
        {
            get
            {
                ...
                return remoteConfig != null && remoteConfig.HasLoadedConfigs && remoteConfig.IsDeveloperDevice();
            }
        }
